...
class HashTable():
    ...
    def remove(self,key):
        hash_code = hash(key) % self.size
        entry = self.table[hash_code]                     # self.table[hash_code] - refers the first entry in linked list assosiated with hash code
        prev = None
        while entry:                                      # Iterate as long as there are entries in the Linked List
            if entry.key == key:                          # Locate entry by comparing target key with key field of entry
                if prev:
                    prev.next = entry.next                # When found, if there is prev reference, link around entry hence removing it from Linked List
                else:
            ...
            prev, entry = entry, entry.next               # if key is not found continue iterating by setting prev to entry and entry to entry.next
